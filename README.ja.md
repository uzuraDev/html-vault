...
> 💡 **AI クライアントから直接アップロード。** 生成した HTML を手作業なしで Vault に保存できます。**Claude Code など**のローカル MCP クライアントは同梱の stdio MCP サーバ経由で、**claude.ai とスマホアプリ**は本体内蔵のリモート MCP エンドポイント経由で。あとは任意の端末で読むだけ。詳細は [MCP 連携](#mcp-連携ヘッドレスアップロード)。
...
**http://localhost:3000** を開き、設定した `AUTH_PASSWORD` でログイン。データを永続化したい場合は任意で `-v $(pwd)/data:/data` を付けます（最小構成では不要）。
...
### B. claude.ai / スマホアプリ — リモート MCP（内蔵）
