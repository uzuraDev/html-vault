...
依存の更新を判断するときは、リスクと見返りを比べてください。セキュリティ上の中身が**無い**
更新が、ローカル実行環境（`wrangler dev` を支える `miniflare` / `workerd`、すなわち
`worker-smoke` テストの土台）に**プレリリース**（`-alpha` / `-beta` / `-rc`）を持ち込むなら、
それは取る価値がありません。安定版を待ちます。Dependabot の PR は必須チェックが緑になり次第
自動マージされるので、`worker-smoke` がブランチ保護に登録されていなければ、これを止めてくれる
仕組みは存在しません。
